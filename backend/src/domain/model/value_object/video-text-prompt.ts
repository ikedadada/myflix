...
export const buildVideoTextPrompt = (params: { tone: VideoTone; userContext?: string }): string => {
  ...
  return [
    ...
    '- JSONでのみ出力し、余計なテキスト・マークダウン・コードフェンス（```）を含めない',
    '- フィールドは title と description のみ。title/description 以外のキーを出力しない',
    `- トーン: ${style}`,
    '',
    contextLine,
