...
export class VideoHandler {
  ...
  stream = async (c: Context<HonoEnv>) => {
    ...
    const content = await this.videoService.findVideoFileObject(authContext.userId, videoId)
    if (!content) {
  ...
  thumbnail = async (c: Context<HonoEnv>) => {
    ...
    const videoId = new VideoId(c.req.param('id'))
    const content = await this.videoService.findThumbnailFileObject(authContext.userId, videoId)
    if (!content) {
