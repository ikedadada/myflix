      ...
      <CardContent className="space-y-4 pt-0">
        <div className="grid gap-4 lg:grid-cols-[2fr_1.2fr]">
          <div className="space-y-3">
            ...
            {objectKey && <p className="text-xs text-success">Uploaded thumbnail key: {objectKey}</p>}
          </div>
          ...
          <div className="space-y-3 rounded-lg border border-border/80 bg-muted/10 p-3">
            <div className="flex items-start justify-between gap-3">
              <div className="space-y-1 text-sm">
                ...
                <p className={`text-xs ${error ? 'text-danger' : 'text-muted-foreground'}`}>{statusText}</p>
                {source === 'manual' && !error && (
                  <p className="text-[11px] text-muted-foreground">手動で設定したサムネイルを使用中です</p>
                )}
              </div>
